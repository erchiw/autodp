...
# RDP bound for bounded range function
# TODO: numerical issue when br<1, or perhaps we use prob * (np.exp(alpha*br) - 1)) and np.exp(alpha*br can be large), like alpha=8000-10000 
# [solved by change initial point to 1e-6]
# TODO: change algebra in log scale
# check if log-sum-exp is ncessary

def bound_gmgf(alpha, br):    
    """
    alpha: rdp order
    br: br parameter
    linear search will fail if set x0=0, alpha=2.35, br=1, 
    """
    
    # original implementation, potential numerical issue
    # def h(t, *args):
    #     """
    #     h function definied in thm 5, Dong et al(2020)
    #     this is the function we want to minimize
    #     """
    #     alpha = args[0]
    #     br = args[1]
    #     prob = ( np.exp(-t) - np.exp(-br) ) / ( 1 - np.exp(-br) ) 
    #     value = alpha * (t - br) + np.log(1 + prob * (np.exp(alpha*br) - 1))
    #     return -value
    
    # change with scipy.special.logsumexp()    
    def h(t, *args):
        """
        h function definied in thm 5, Dong et al(2020)
        this is the function we want to minimize
        ...
        term1 = alpha * (t - br)
        term2 = scipy.special.logsumexp(a=[br, alpha*br, br-t, alpha*br+br-t], b=[1, -1, -1, 1])
        term3 = -scipy.special.logsumexp(a=[br, 0], b=[1, -1])
        
        value = term1 + term2 + term3
        return -value

    opt_pt = fmin_tnc(h, args=(alpha, br), x0=np.array([1e-6]), approx_grad=True, bounds=[(0,br)])
    value = -h(opt_pt[0], alpha, br)
    return value[0]


def RDP_BR(params, alpha):
    """
    This function calaulate RDP based on bounded range parameter.
    """    
    br = params['br']
    ...
    if alpha <= 1:
        tmp = br / (np.exp(br)-1)
        return (tmp - 1 - np.log(tmp))
    elif np.isinf(alpha):
        return br
    elif alpha > 1:
        return bound_gmgf(alpha, br) / (alpha-1)
