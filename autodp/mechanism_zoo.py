...
class ExponentialMechanism(zCDP_Mechanism):
    # expoMech supports bounded range characterization
    # x ~ exp(eps*u / (2*senstivity(u))
    def __init__(self, eps, RDP_off=False, BR_off=True, phi_off=True, non_adaptive=False, name='ExpMech'):
        ...
        if not RDP_off:
            if not BR_off:
                new_rdp = lambda x: rdp_bank.RDP_BR({'br': eps}, x)
                self.propagate_updates(new_rdp, 'RDP')
            else:
...
# Bounded range mechanism

class BR_Mechanism(zCDP_Mechanism):
    # general bounded range mechanism, eps-br implies eps-dp (while eps-dp implies 2eps-br)
      def __init__(self, br, RDP_off=False, BR_off=False, phi_off=True, non_adaptive=False, name='BR_Mech'):
        zCDP_Mechanism.__init__(self, br**2/8, name=name)
        # the RDP bound is from here: https://arxiv.org/abs/1909.13830

        self.br = br
        self.eps_pureDP = br
        self.propagate_updates(br, 'pureDP')

        if not RDP_off:
            if not BR_off:
                new_rdp = lambda x: rdp_bank.RDP_BR({'br': br}, x)
                self.propagate_updates(new_rdp, 'RDP')
            else:
                new_rdp = lambda x: rdp_bank.RDP_pureDP({'eps': br}, x)
                self.propagate_updates(new_rdp, 'RDP')
